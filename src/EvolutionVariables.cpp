...
Spacetime::Spacetime() : csf_model(this), rsf_model(this) {
    ...
    // Initialize scalar-field energies and fluxes
    E_phi = 0.0;
    E_psi = 0.0;
    E_flux_phi = 0.0;
    E_flux_psi = 0.0;
...
// Compute energy fluxes E_phi (complex scalar) and E_psi (real scalar) at large radius 
void Spacetime::compute_energy_fluxes(BSSNSlice* slice_ptr)
{
    if (slice_ptr == nullptr || slice_ptr->states2.empty())
    {
        E_flux_phi = 0.0;
    ...
    double e_flux_phi = 0.0;
    double e_flux_psi = 0.0;

    // Integrate over sphere at large radius 
    const int j = static_cast<int>(0.95 * N) - 1;
    const double z = (j + offset) * dr_local;
    const BSSNState& bssn = slice_ptr->states2[j].bssn;
    const double chi = std::max(min_chi, bssn.chi);
    ...
    // Complex scalar field contribution
    {
        const CSF csf = slice_ptr->states2[j].csf;
        const CSF d_z_csf{slice_ptr->d_z(v_phi_re, j), slice_ptr->d_z(v_phi_im, j), 0.0, 0.0};
        e_flux_phi = 2 * sphere_area * std::pow(z, D - 2.) * std::pow(chi, 0.5 * (1.0 - D)) * (csf.K_phi_re * d_z_csf.phi_re + csf.K_phi_im * d_z_csf.phi_im);
    }
        ...
        if (add_real_field)
        {
            const RSF rsf = slice_ptr->states2[j].rsf;
            const RSF d_z_rsf{slice_ptr->d_z(v_psi, j), slice_ptr->d_z(v_K_psi, j)};
            e_flux_psi = 2 * sphere_area * std::pow(z, D - 2.) * std::pow(chi, 0.5 * (1.0 - D)) * (rsf.K_psi * d_z_rsf.psi);
        }

    E_flux_phi = e_flux_phi;
    E_flux_psi = add_real_field ? e_flux_psi : 0.0;
}

// Placeholder: 4D Ricci scalar at grid center (to be completed with the correct formula).
double Spacetime::ricci_4_ctr() const {
    return ricci_4_val(0); 
}
...
void Spacetime::evolve()
{
    ...
    if (!actr_evol_file)
    {
        std::cerr << "evol_variables_p" << std::setprecision(15) << real_amp << ".dat could not be opened for writing!\n";
        exit(1);
    }
    actr_evol_file << "#" << std::setw(20) << "time "
        ...
        << std::setw(21) << "CSF_flux"
        << std::setw(21) << "RSF_flux"
        << std::setw(21) << "alpha "
        << std::setw(21) << "chi "
        << std::setw(21) << "hzz "
        << std::endl;
    ...
    for (int time_step = 0; time_step < num_timesteps; time_step++)
    {
        ...
        double phase_ctr = std::arg( std::complex<double>(slices[n].states2[0].csf.phi_re, slices[n].states2[0].csf.phi_im));

        double omega_approx = 0.0;
        if (time_step > 0) {
            omega_approx = (phase_ctr - phase_last) / dt;
            if (omega_approx < -M_PI / dt) omega_approx += 2 * M_PI / dt;
        }
        phase_last = phase_ctr;
        ...
        if (time_step % write_CN_interval == 0) //write time-dependent diagnostics to constraint_norms.dat
        {
            ...
            compute_energy_fluxes(current_slice_ptr);
            ...
            actr_evol_file << std::fixed << std::setprecision(10)
                ...
                << std::setw(20) << E_phi << " "
                << std::setw(20) << E_psi << " "
                << std::setw(20) << E_flux_phi << " "
                << std::setw(20) << E_flux_psi << " "
            ...
            if (n_output_1d > 0 && time_step % output_1d_every == 0) 
            {
                ...
                for (int var_ind=0; var_ind<n_output_1d+1; var_ind++)
                {
                    ...
                    // write radial data
                    for (int r_ind = 0; r_ind < (int) slices[n].states2.size(); r_ind++)
                    {
                        ...
                        if (output_1d_var[var_ind] == "radii") {
                            ...
                            if (add_real_field) {
                                ...
                                const RSF d_z_rsf{slices[n].d_z(v_psi, r_ind), slices[n].d_z(v_K_psi, r_ind)};
                                const RSF d_zz_rsf{0.0, 0.0};
